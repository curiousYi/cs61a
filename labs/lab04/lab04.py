...
def distance(city1, city2):
    ...
    city1Lat, city1Lon = get_lat(city1), get_lon(city1)
    city2Lat, city2Lon = get_lat(city2), get_lon(city2)
    
    return sqrt((city1Lat-city2Lat)**2 + (city1Lon-city2Lon)**2)
...
def closer_city(lat, lon, city1, city2):
    ...
    dummy_city = make_city('SomeCity', lat, lon)

    distance_to_city1 = distance(dummy_city, city1)
    distance_to_city2 = distance(dummy_city, city2)
...
def put_piece(board, max_rows, column, player):
    ...
    new_board = []
    row_where_piece_ends_up = -1
    has_piece_been_set = False

    for i in range(max_rows-1, -1, -1):
        if board[i][column] == '-' and not has_piece_been_set:
            new_row = replace_elem(board[i], column, player)
            new_board = [new_row] + new_board
            row_where_piece_ends_up = i
            has_piece_been_set = True
        else:
            new_board = [board[i]] + new_board
    
    return row_where_piece_ends_up, new_board
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    is_valid, new_board = put_piece(board, max_rows, col, player)
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows-1, -1, -1):
        ...
        for col in range(0, max_cols):
            printed_row = printed_row + board[row][col]
            if col != max_cols-1:
                printed_row = printed_row + ' '
            elif row != max_rows - 1 and col == max_cols -1:
                printed_row = printed_row + '\n'
        
        printed_board = printed_row + printed_board
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    if num_connect > max_cols:
        return False

    first_of_num_connect = False
    adjacent_player_so_far = 0
    for i in range(0, max_cols-num_connect+1):
        if not first_of_num_connect and board[row][i] == player:
            first_of_num_connect = True
            adjacent_player_so_far = 1
        elif first_of_num_connect and board[row][i] == player:
            adjacent_player_so_far = adjacent_player_so_far + 1
            if(adjacent_player_so_far == num_connect):
                return True
